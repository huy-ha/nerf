...
class Reptile:
    ...
    allowed to leak between test samples via Batch~Norm.
    ...
    def train_step(self,
        ...
        for _ in range(meta_batch_size):
            mini_dataset = _sample_mini_dataset(
                dataset, num_classes, num_shots)
            for batch in _mini_batches(mini_dataset, inner_batch_size, inner_iters, replacement):
                ...
                self.session.run(minimize_op, feed_dict={
                                 input_ph: inputs, label_ph: labels})
    ...
    def evaluate(self,
        ...
        for batch in _mini_batches(train_set, inner_batch_size, inner_iters, replacement):
            ...
            self.session.run(minimize_op, feed_dict={
                             input_ph: inputs, label_ph: labels})
        test_preds = self._test_predictions(
            train_set, test_set, input_ph, predictions)
        num_correct = sum([pred == sample[1]
                           for pred, sample in zip(test_preds, test_set)])
    ...
    def _test_predictions(self, train_set, test_set, input_ph, predictions):
        ...
        for test_sample in test_set:
            ...
            res.append(self.session.run(
                predictions, feed_dict={input_ph: inputs})[-1])
        return res
